package com.jdyna.players.jedrzejczaknowak.state;
...
public class GameState {
     ...
     * 
     * @return found player, or <code>null</code> if no player with the given
     *         name exists
     */
    ...
    public boolean isPointOutsideBoard(Point p) {
        if (p.x < 0 || p.y < 0 || p.x >= spaces.length
                || p.y >= spaces[0].length) {
            return true;
        }
        return false;
     ...
     * 
     * @param x
     *            horizontal grid coordinate (top->down)
     * @param y
     *            vertical grid coordinate (left->right)
     */
    ...
    public void update(int currentFrame, GameStateEvent event) {
        ...
        for (int i = 0; i < spaces.length; ++i) {
            for (int j = 0; j < spaces[i].length; ++j) {
                ...
                if (space.getBomb() != null) {
                    space.getBomb().setTimer(
                            space.getBomb().getTimer() - frameDifference);
                }
        ...
        for (int i = 0; i < spaces.length; ++i) {
            for (int j = 0; j < spaces[i].length; ++j) {
                ...
                } else if (cell.type.isExplosion()
                        || cell.type == CellType.CELL_CRATE_OUT) {
                    if (space.isSafe()) {
     ...
     * 
     * @return found player, or <code>null</code> if all players are either dead
     *         or immortal
     */
    private Player findClosestPlayer(Space space) {
        ...
        for (Player player : players) {
            if (player.getState() == PlayerState.PWNING
                ...
                if (spaces[playerGridPosition.x][playerGridPosition.y] == space) {
                    distance = Math.max(Math.abs(center.x
                            - player.getPosition().x), Math.abs(center.y
     ...
     * <p>
     * This method automatically calculates the range, estimated time of
     * detonation and spaces that will be affected by this bomb's explosion. If
     * any of those spaces contain a bomb scheduled to detonate later than this
     * bomb, that bomb is disarmed and replanted to ensure proper explosion
     * estimation.
     * 
     * @param owner
     *            player who plants the bomb
     * @param location
     *            space the bomb is being planted on
     * @param compensation
     *            number of frames lost between this frame and the last frame
     *            received
     */
    private void dropBomb(Player owner, Space location, int compensation) {
        ...
        affectedSpaces.addAll(propagateExplosion(location.x + 1, location.y,
                location.x + bomb.range, location.y, detonationTime));
        affectedSpaces.addAll(propagateExplosion(location.x - 1, location.y,
                location.x - bomb.range, location.y, detonationTime));
        affectedSpaces.addAll(propagateExplosion(location.x, location.y + 1,
                location.x, location.y + bomb.range, detonationTime));
        affectedSpaces.addAll(propagateExplosion(location.x, location.y - 1,
                location.x, location.y - bomb.range, detonationTime));
    ...
    /**
     * Propagates the explosion from <code>Space (x1, y1)</code> to
     * <code>(x2, y2)</code>.
     * 
     * @param timer
     *            time left until the explosion
     * @return a list of spaces affected by the explosion
     */
    private List<Space> propagateExplosion(int x1, int y1, int x2, int y2,
            int timer) {
        ...
        for (int i = x1; xd * i <= xd * x2; i += xd) {
            for (int j = y1; yd * j <= yd * y2; j += yd) {
                ...
                if (inRange.getType() == SpaceType.CRATE
                        && (inRange.getNextExplosion() == null || inRange
                                .getNextExplosion() >= timer)) {
