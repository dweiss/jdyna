...
public class TZPlayerServant extends ICPlayerControllerPOA {
	/*
	 * Board Informations
	 */
	 ...
	 * @param playerName
	 */
	public TZPlayerServant(String name) {
	...
	public synchronized void onEnd(CGameResult result) {
		// TODO odrejestrowac sie z corby
	}

	@Override
	public synchronized void onFrame(int frame, CGameEvent[] events) {
		List<GameEvent> gameEvents = Adapters.adapt(events, cBoardInfo, cPlayers);
		this.bomb = false;
		for (int i = 0; i < gameEvents.size(); i++) {
			if (gameEvents.get(i).type.equals(GameEvent.Type.GAME_START)) {
				this.boardInfo = ((GameStartEvent) gameEvents.get(i)).getBoardInfo();
				this.direction = null;
				this.target = null;
			} else if (gameEvents.get(i).type.equals(GameEvent.Type.GAME_STATE)) {
				GameStateEvent gameState = (GameStateEvent) gameEvents.get(i);
				final GameStateEvent gse = (GameStateEvent) gameEvents.get(i);
	        	...
	        	synchronized (lock){
	        		//System.out.println("SIZe : " + gse.getPlayers().size() );
	                final IPlayerSprite myself = identifyMyself(gse.getPlayers());
	                Cell cells[][] = ((GameStateEvent) gameEvents.get(i)).getCells(); 
