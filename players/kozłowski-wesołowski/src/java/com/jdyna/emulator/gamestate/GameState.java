...
import com.jdyna.emulator.gamestate.bombs.OpponentsSimulatedBombs;
import com.jdyna.emulator.gamestate.bombs.SimulatedBombs;
...
public class GameState {
	 ...
	 */
	public GameState(final int frame, final GameStateEvent event, final String currentPlayer) {
		lastFrame = frame - 1;
	...
	/**
	 * Returns <code>true</code> if a player can walk on the grid's given coordinates. Copied from com.jdyna.Game
	 * class and adapted.
	 */
...
//		}
	...
	public boolean isATunnel(final GridCoord start, final GridCoord destination, final Direction d) {
	...
	public boolean isLineWalkable(final GridCoord start, final GridCoord destination, final Direction d) {
		GridCoord next = board.nextCell(start, d);
		while (!destination.equals(next) && board.cellAt(next).isWalkable()) {
			next = board.nextCell(next, d);
		}
	...
	/**
	 * Returns <code>true</code> if frames from given period are safe. Considers bombs on opponents positions.
	 * 
	 * @param from First frame of period (inclusive).
	 * @param to One before last frame of period.
	 */
	public boolean isPeriodSafeWithOppBombs(final PointCoord point, final int from, final int to) {
		final int timeDiff = to - 1 - from;
		if (timeDiff <= Bombs.EXPLOSION_FRAMES) {
	 ...
	 * @param point To check its safety.
	 * @param framesShift Describes future in number of frames.
	 * @return <code>true<code> if cell will be safe, otherwise <code>false</code>. Considers bombs on all players positions.
	 */
	 ...
	 * @param point To check its safety.
	 * @param framesShift Describes future in number of frames.
	 * @return <code>true<code> if cell will be safe, otherwise <code>false</code>. Considers bombs on opponents positions.
	 */
	...
	/**
	 * @param point To check its safety.
	 * @return <code>true<code> if cell is ultimately safe (without any bombs in future), otherwise <code>false</code>. Considers bombs on all players positions.
	 */
	...
	/**
	 * @param point To check its safety.
	 * @return <code>true<code> if cell is ultimately safe (without any bombs in future), otherwise <code>false</code>. Considers bombs on opponents positions.
	 */
	...
	/**
	 * Returns first safe frames after explosions. Considers bombs on all players positions.
	 */
	public List<Integer> safeFramesWithOppAndMyBombs(final PointCoord point, final int elapsedFrames) {
		final List<Integer> result = Lists.newLinkedList();
		int lastExplosion = elapsedFrames;
		final int maxFrame = Math.max(elapsedFrames + 1, Bombs.BOMB_LIFETIME + 1);
		for (int frame = elapsedFrames + 1; frame <= maxFrame; frame++) {
	 ...
	 */
	public List<Integer> safeFramesWithOppBombs(final PointCoord point, final int elapsedFrames) {
		final List<Integer> result = Lists.newLinkedList();
		int lastExplosion = elapsedFrames;
		final int maxFrame = Math.max(elapsedFrames + 1, Bombs.BOMB_LIFETIME + 1);
		for (int frame = elapsedFrames + 1; frame <= maxFrame; frame++) {
	...
	public void update(final int frame, final GameStateEvent event) {
		...
		if (nrOfFrames <= 0) {
			logger.warn("Frame with id from past was ignored, lastFrame: " + lastFrame + ", newFrame: " + frame);
			return;
		}
