...
/**
 * <p>
 * Updates information on cells threatened by bomb explosion, that don't contain that bomb.
 * </p>
 * 
...
final class ZoneSafetyUpdater {
	...
	public void update(final int nrOfFrames, final Board board, final Bombs bombs) {
		...
		final Map<GridCoord, Integer> uncheckedBombs = this.bombs.getReadyBombsWithTimers();
	 ...
	 * @return <code>true</code> if at least one bomb that threaten this cell will be after explosion.
	 */
	...
	private void addBombZone(final GridCoord grid, final GridCoord bombLocation, final BombState bzs) {
	...
	private void checkBombDirection(final GridCoord grid, final int range, final Direction direction) {
	...
	private void checkExplosionDirection(final GridCoord gird, final int range, final Direction direction) {
	...
	private void updateExplodedBomb(final int nrOfFrames, final GridCoord gird, final GridCoord bombLocation) {
		addBombZone(gird, bombLocation, bombs.getBombState(bombLocation));
	}

	private void updateReadyBomb(final int nrOfFrames, final GridCoord grid, final GridCoord bombLocation) {
	...
	private int distance(final GridCoord first, final GridCoord second, final Direction direction) {
		final int dist;
		switch (direction) {
		case DOWN:
