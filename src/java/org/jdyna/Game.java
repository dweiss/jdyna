...
/**
 * Game controller. The controller and <b>all the objects involved in the
 * game</b> are single-threaded and should be accessed from within the game loop
 * only.
 */
public final class Game implements IGameEventListenerHolder {
	...
	public static enum Mode {
		/**
		 * In last man standing mode ("classic" Dyna Blaster), the purpose of
		 * the game is to kill all the enemies. The last player to survive wins
		 * the game. All players are ranked after the game is over.
		 * 
		 * @see GameResult
		 * @see Standing
		 */
		...
		/**
		 * In the death match mode, players compete by bombing each other. When
		 * player A kills player B, then player A gets a reward. It does not
		 * matter whose bomb initiated the explosion, the origin (bomb) of any
		 * 'flame' that kills player B earns player A a credit. In case of
		 * overlapping flames, the credit is given to all players that
		 * contributed to overlapping flame. The game ends when there is only
		 * one player (or zero players) that are still alive. The number of
		 * lives (reincarnations) for each player is controlled separately.
		 */
		DEATHMATCH,

		/**
		 * Same as {@link #DEATHMATCH}, but the game never finishes (even if
		 * there are no players on the board).
		 */
		INFINITE_DEATHMATCH
	}

	/**
	 * Dynamic information about players involved in the game. Indexed
	 * identically to {@link #players}
	 */
	private final List<PlayerInfo> playerInfos = Lists.newArrayList();

	/** Game listeners. */
	private final ArrayList<IGameEventListener> listeners = Lists
			.newArrayList();

	/** Frame listeners. */
	private final ArrayList<IFrameListener> frameListeners = Lists
			.newArrayList();
	...
	/**
	 * If periodic bonuses are placed on the board, then this is the period
	 * after which a new bonus should be placed on the board. Bonuses will be
	 * placed at random board positions.
	 */
	...
	/**
	 * Dynamically attach a new player to an existing game. If the player with
	 * the given identifier already exists, an exception is thrown.
	 */
	...
	/**
	 * Starts the game, does not return until the game ends. The game can be
	 * interrupted by setting the running thread's interrupted flag.
	 */
	public GameResult run(Mode mode) {
		...
		do {
			...
			/*
			 * No player-related data structure fiddling while within frame
			 * processing.
			 */
			...
			synchronized (this) {
				...
				/*
				 * Check if player status should be dispatched. Dispatch every
				 * 50 frames or so anyway, so that clients that have just joined
				 * the game have their status updated.
				 */
				...
				/*
				 * The game may be finished, but there are still lingering
				 * frames we must replay.
				 */
				if (result == null) {
		...
		if (interrupted || Thread.interrupted()) {
			if (result == null)
				result = new GameResult(mode, getPlayerStats());
	...
	private void processBonuses(int frame) {
		if (nextBonusFrame < frame) {
			/*
			 * We pick the bonus location at random, avoiding cells where
			 * players are. We could try to make it smarter by placing bonuses
			 * in equal or at least maximum distance from all players, but it
			 * did not work that well in practice (I tried).
			 */
			final HashSet<Point> banned = Sets.newHashSet();
			for (PlayerInfo pi : playerInfos) {
				if (!pi.isDead())
					banned.add(boardData.pixelToGrid(pi.location));
			}

			final Point p = randomEmptyCell(banned);
			if (p != null) {
				final CellType bonus = BONUSES.get(random.nextInt(BONUSES
						.size()));
	...
	private Point randomEmptyCell(Collection<Point> banned) {
		final ArrayList<Point> positions = Lists
				.newArrayListWithExpectedSize(board.width * board.height / 2);
		...
		if (nextCrateFrame < frame) {
			...
			for (PlayerInfo pi : playerInfos) {
				if (!pi.isDead()) {
					...
					banned.add(point);
					banned.addAll(BoardUtilities.findBlockingLocations(board,
							point));
				}
			}
			...
			for (Point point : board.defaultPlayerPositions) {
				banned.add(point);
				banned.addAll(BoardUtilities
						.findBlockingLocations(board, point));
			}
	...
	private List<TeamStatus> getTeamStats() {
		...
		final ArrayList<TeamStatus> stats = Lists
				.newArrayListWithExpectedSize(teams.size());
	...
	public void addListener(IGameEventListener listener) {
		if (listeners.contains(listener)) {
			throw new RuntimeException(
					"It is an error to add the same listener more than once: "
							+ listener);
		}
	...
	/**
	 * Interrupt the currently running game. The interrupted game may be delayed
	 * for the duration of a frame to allow dispatching finalizing events.
	 */
	...
	private void fireFrameEvent(int frame) {
		...
		for (IGameEventListener gl : listeners) {
			try {
				...
				logger.error("On-frame exception from listener: "
						+ gl.getClass(), t);
			}
		}
	}
	...
	/**
	 * Move players according to their controller signals, drop bombs, check
	 * collisions.
	 */
	private void processPlayers(int frame) {
		...
		for (PlayerInfo pi : playerInfos) {
			...
			if (c.dropsBomb()
					&& (!pi.isImmortal() || pi.immortalityBonusCollected)) {
	 ...
	 * Check collisions against bombs and other active cells.
	 */
	private void checkCollisions(int frame, List<PlayerInfo> kills,
			PlayerInfo pi) {
		/*
		...
		/*
		 * Check collisions against grid cells. We only care about the cell
		...
		if (c.type.isLethal()) {
			...
			/*
			 * If the cell below is an explosion update attributions for this
			 * fatality.
			 */
		...
		/*
		 * Process bonuses. The bonus-assignment is not entirely fair, because
		 * if two players touch the bonus at once, the player with lower index
		 * will collect the bonus. With randomized player order, however, this
		 * should be of no practical importance.
		 */
		...
		if (c.type == CellType.CELL_BONUS_CRATE_WALKING) {
			pi.crateWalkingEndsAtFrame = frame
					+ Globals.DEFAULT_CRATE_WALKING_FRAMES;
			pi.canWalkCrates = true;
			bonusCollected = true;
		}
		
		if (c.type == CellType.CELL_BONUS_BOMB_WALKING) {
			pi.bombWalkingEndsAtFrame = frame
					+ Globals.DEFAULT_BOMB_WALKING_FRAMES;
	...
	/**
	 * Attempt to drop a bomb at the given location (if the player has any bombs
	 * left and the cell under its feet is empty).
	 */
	...
	/**
	 * The movement-constraint code below has been engineered by trial and error
	 * by looking at the behavior of the original Dyna Blaster game. The basic
	 * logic is that we attempt to figure out the "target" cell towards which
	 * the player is heading and "guide" the player's coordinates towards the
	 * target. This way there is a possibility to run on diagonals (crosscut
	 * along the edge of a cell).
	 */
	private void movePlayer(PlayerInfo pi, Direction signal) {
		...
		if (max(abs(rx), abs(ry)) <= boardData.cellSize) {
			if (!canWalkOn(pi, txy)) {
				/*
				 * We try to perform 'easing', that is moving the player towards
				 * the cell from which he or she will be able to move further.
				 */
	...
	/**
	 * A helper function that tests if we can apply easing in one of the
	 * directions. This is generalized for all the possibilities, so it may be
	 * vague a bit.
	 */
	private boolean ease(PlayerInfo pi, Point xy, int o, int x1, int y1,
			int x2, int y2, Direction d1, int x3, int y3, int x4, int y4,
			Direction d2) {
	...
	/**
	 * Returns <code>true</code> if a player can walk on the grid's given
	 * coordinates.
	 */
	private boolean canWalkOn(PlayerInfo pi, Point txy) {
		/*
		 * Players in immortality mode can walk over bombs, but not anything
		 * else.
		 */
	...
	private void setupPlayer(Player p, int immortalityCount) {
		...
		final PlayerInfo pi = new PlayerInfo(p, initialLives, spriteType,
				currentFrame);
	...
	private void processBoardCells() {
		...
		for (int x = board.width - 1; x >= 0; x--) {
			for (int y = board.height - 1; y >= 0; y--) {
				...
				/*
				 * Advance counter frame on cells that use it. Clean up cells
				 * that have finished animating.
				 */
				cell.counter++;
