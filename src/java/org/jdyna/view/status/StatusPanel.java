...
public class StatusPanel extends JPanel implements IGameEventListener
{
    ...
    private void initializeComponents()
    {
        ...
        statuses[3] = new Status("Max range", getCellImage(CellType.CELL_BONUS_MAXRANGE,
            0), -1);
        statuses[4] = new Status("Ahmed", getCellImage(CellType.CELL_BONUS_AHMED, 0), -1);
        statuses[5] = new Status("Speed up",
            getCellImage(CellType.CELL_BONUS_SPEED_UP, 0), -1);
        statuses[6] = new Status("Crate walking", getCellImage(
            CellType.CELL_BONUS_CRATE_WALKING, 0), -1);
        statuses[7] = new Status("Bomb walking", getCellImage(
            CellType.CELL_BONUS_BOMB_WALKING, 0), -1);
        statuses[8] = new Status("Immortality", getCellImage(
            CellType.CELL_BONUS_IMMORTALITY, 0), -1);

        statuses[9] = new Status("Diarrhea",
            getCellImage(CellType.CELL_BONUS_DIARRHEA, 0), -1);
        statuses[10] = new Status("No bombs", getCellImage(CellType.CELL_BONUS_NO_BOMBS,
            0), -1);
        statuses[11] = new Status("Slow down", getCellImage(
            CellType.CELL_BONUS_SLOW_DOWN, 0), -1);
        statuses[12] = new Status("Controller reverse", getCellImage(
            CellType.CELL_BONUS_CONTROLLER_REVERSE, 0), -1);

        for (int i = 0; i < statuses.length; i++)
        {
            if (statuses[i].getName() == "Max range"
                || statuses[i].getName() == "Diarrhea") gbc.insets = new Insets(1, 5, 1,
                1);
            add(statuses[i], gbc);
            gbc.insets = new Insets(1, 1, 1, 1);
            gbc.gridx++;
