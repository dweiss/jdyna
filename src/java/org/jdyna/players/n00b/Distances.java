...
/**
 * Contains distances to the interesting spaces on the board computed for the specified
 * starting point.
 * <p>
 * All fields of type <code>Integer</code> can be <code>null</code>. This means that there
 * is no enemy/bonus/etc... on board.
 */
public class Distances
{
    ...
    private void findClosestEnemy()
    {
        ...
        else
        {
            ...
            {
                if (me.equals(player) || player.getState() != PlayerState.PWNING)
                {
                    continue;
                }
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                if (distances[x][y] == Integer.MAX_VALUE)
                {
                    ...
                    {
                        int dist = BoardUtilities.manhattanDistance(state.getPlayerSpace(
                            me).toPoint(), s.toPoint());
                ...
                // safe
                if (s.getType() != SpaceType.CRATE && s.getType() != SpaceType.WALL
                    && !s.isHot() && s.isSafe())
                {
    ...
    /**
     * Returns the neighbour reachable from the given point by walking one space in the
     * given direction.
     */
    public static Point getNeighbourPointByDirection(Point p, Direction direction)
    {
        if (direction == null)
        {
            return p;
    ...
    /**
     * Checks if there is no barrier between <code>Space a</code> and <code>b</code>.
     * 
    ...
    public String toString()
    {
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                {
                    builder.append(String.format(Locale.US, "%2d", distances[i][j]));
                }
                builder.append(" ");
            }
            builder.append('\n');
