 ...
 */
public final class SmartBomber extends AbstractPlayerEmulator implements
    IPlayerController2
{
    ...
    /**
     * Number of the same directions at the beginning of the path. Used to implement
     * factories where you can explicitly define the number of frames to move in the same
     * direction.
     */
    ...
    public Direction getCurrent()
    {
        if (state != null && state.amIAlive())
        {
            ...
            if (closestOpponentsName != null && state.isPlayerAlive(closestOpponentsName))
            {
                ...
                for (GridCoord neighbor : state.getBoard().getWalkableNeighbors(
                    opponentsCell))
                {
                    final List<Direction> p = pathfinder.findTrip(state, state
                        .getPlayerPosition(), neighbor, minDistance);
                    if (p == null)
                    {
                        continue;
                    }
            ...
            {
                final List<Direction> p = pathfinder.findTrip(state, state
                    .getPlayerPosition(), bonusCell, minDistance);
                if (p == null)
                {
                    continue;
                }
            ...
            {
                path = pathfinder
                    .findShelterDesperately(state, state.getPlayerPosition());
            }
